  ...
  return <View style={styles.screen}>
    ...
    {loading ? <LoadingState label="Đang tải sự kiện…" /> : error ? <ErrorState message={error} onRetry={() => setReloadKey((value) => value + 1)} /> :
      <FlatList contentContainerStyle={events.length ? styles.list : styles.empty} data={events} keyExtractor={(item) => item.id}
        ...
        renderItem={({ item }) => <Pressable accessibilityRole="button" onPress={() => router.push({ pathname: "/events/[id]", params: { id: item.id } })} style={styles.card}>
          <View style={styles.row}><Text style={styles.cardTitle}>{item.title}</Text>{item.registration.isRegistered && <Text style={styles.badge}>Đã đăng ký</Text>}</View>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: "#fff", borderRadius: 16, elevation: 2, padding: 18, shadowColor: "#172554", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 12 }, row: { alignItems: "flex-start", flexDirection: "row", gap: 8, justifyContent: "space-between" }, cardTitle: { color: "#172554", flex: 1, fontSize: 18, fontWeight: "700" }, badge: { backgroundColor: "#dcfce7", borderRadius: 8, color: "#166534", fontSize: 11, fontWeight: "700", paddingHorizontal: 8, paddingVertical: 5 }, meta: { color: "#475569", fontSize: 14, marginTop: 7 }, description: { color: "#64748b", fontSize: 14, lineHeight: 20, marginTop: 8 },
});
